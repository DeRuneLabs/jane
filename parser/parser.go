...
func (p *Parser) processSingleValuePart(token lexer.Token, builder *expressionModelBuilder) (v value, ok bool) {
...
func canGetPointer(v value) bool {
	...
	return v.ast.Token.Id == lexer.Name || typeIsArray(v.ast.Type)
}

func (p *Parser) processValuePart(tokens []lexer.Token, builder *expressionModelBuilder) (v value) {
	if tokens[0].Id == lexer.Operator {
...
func (p *Parser) processParenthesesValuePart(tokens []lexer.Token, builder *expressionModelBuilder) (v value) {
...
func (p *Parser) processBraceValuePart(tokens []lexer.Token, builder *expressionModelBuilder) (v value) {
	var valueTokens []lexer.Token
	j := len(tokens) - 1
	braceCount := 0
	for ; j >= 0; j-- {
...
func (p *Parser) processBracketValuePart(tokens []lexer.Token, builder *expressionModelBuilder) (v value) {
	var valueTokens []lexer.Token
	j := len(tokens) - 1
	braceCount := 0
	for ; j >= 0; j-- {
...
func (p *Parser) getRangeTokens(open, close string, tokens []lexer.Token) []lexer.Token {
	braceCount := 0
	start := 1
	for index, token := range tokens {
		if token.Id != lexer.Brace {
...
func (p *Parser) checkReturns(fun ast.FunctionAST) {
	...
	if missed && fun.ReturnType.Code != jn.Void {
...
func (p *Parser) processFuncMultiVarset(vsAST *ast.VariableSetAST, funcVal value) {
	...
	if len(types) != len(vsAST.SelectExpressions) {
		p.PushErrorToken(vsAST.Setter, "missing_multiassign_identifier")
		return
	}
	for index := range vsAST.SelectExpressions {
		selector := &vsAST.SelectExpressions[index]
